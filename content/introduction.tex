...
We use Chapter \ref{chapter:multisignatures} to describe a multisignature method that allows multiple people to send and receive money collaboratively. %This is formally called (N-1)-of-N and N-of-N threshold authentication.
...
We\marginnote{Isn't this useful?} use margin notes to indicate where Monero implementation details can be found in the source code. There is usually a file path, such as src/ringct/rctOpps.cpp, and a function, such as \(\textrm{signMultisig()}\). Note: any instance of `-' indicates split text, such as crypto- note $\rightarrow$ cryptonote.
