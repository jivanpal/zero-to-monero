...
The output of a hash function $\mathcal{H}$ is uniformly distributed across the range of possible outputs. In other words, for some input $A$, $\mathcal{H}(A) \in^D_R \mathbb{S}_H$ where $\mathbb{S}_H$ is the set of possible outputs from $\mathcal{H}$. We use $\in^D_R$ to indicate the function is deterministically random. $\mathcal{H}(A)$ produces the same thing every time, but its output is equivalent to a random number.
...
Including\marginnote{src/ringct/ rctOps.cpp} the computation of \((\sum\limits_j C_{i, j} - \sum\limits_t C^b_{t}) - f H\), and verifying key images are in $G$'s subgroup with $l \tilde{K}$, we verify a {\tt RCTTypeFull} transaction's MLSAG (Section \ref{sec:MLSAG}) with:

\begin{itemize}
    \setlength\itemsep{\listspace}
    \item [\textbf{PA}] Point addition for some points $A, B$: $A + B$ \quad \([(m-1)*(v+1) + p]\)%commitment sum 1, sum 2, fee
    \item [\textbf{PS}] Point subtraction for some points $A, B$ : $A - B$ \quad \([v+1]\)%commitment sub
    \item [\textbf{VBSM}] Variable-base scalar multiplications for some integer $a$, and point $P$: $a P$ \quad [$m$ + 1]%verify key images are in generator subgroup fee fH
...
From\marginnote{src/ringct/ ringCT.cpp genBorromean()} Section \ref{sec:borromean}, Section \ref{sec:range_proofs}, and Section \ref{full-signature} we know that a Monero Borromean signature for range proofs takes the form of an n-tuple%genBorromean in ringCT.cpp
...
Including\marginnote{src/ringct/ rctOps.cpp} the computation of \( (C_{i, j} - C'^a_{\pi, j}) \) and \( (\sum_j C'^a_{j} - \sum_t C^b_{t}) - f H \), and verifying key images are in $G$'s subgroup with $l \tilde{K}$, we verify all of a {\tt RCTTypeSimple} transaction's MLSAGs with:

\begin{itemize}
    \setlength\itemsep{\listspace}
    \item [\textbf{PA}] Point addition for some points $A, B$: $A + B$ \quad \([(m-1) + p]\)%commitment sum 1, sum 2, fee
...
\begin{itemize}
    \item \underline{Type}: `0' is {\tt Miner Transaction}, `1' is {\tt RCTTypeFull}, and `2' is {\tt RCTTypeSimple} %see chapter 7, blockchain, about type 0 transactions
...
For\marginnote{src/ringct/ rctOps.cpp} {\tt RCTTypeFull} we need $((v + 1) \cdot (m + 1) + m + 1) \cdot 32$ bytes of storage, and verify with:
\begin{itemize}
    \setlength\itemsep{\listspace}
    \item [\textbf{PA}] Point addition for some points $A, B$: $A + B$ \quad \([(m-1)*(v+1) + p]\)%commitment sum 1, sum 2, fee
    \item [\textbf{PS}] Point subtraction for some points $A, B$ : $A - B$ \quad \([v+1]\)%commitment sub
    \item [\textbf{VBSM}] Variable-base scalar multiplications for some integer $a$, and point $P$: $a P$ \quad [$m$ + 1]%fee fH
    \item [\textbf{DVBA}] Double-variable-base addition for some integers $a, b$, and point $B$: $a G + b B$ \quad \([(m+1)*(v+1)]\)%mlsag
    \item [\textbf{VBA}] Variable-base additions for some integers $a, b$, and points $A, B$: $a A + b B$ \quad \([m*(v+1)]\)%mlsag, no key image for commitments
\end{itemize}
